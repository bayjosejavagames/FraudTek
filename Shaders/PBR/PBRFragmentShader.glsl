...
void main(void){
    ...
    for(int i = 0; i < 4; i++){
        float distance = length(toLightVector[i]+(roughnessColor.xyz * 2));
        ...
        float specularAddition = (dot(reflect(-normalize(toLightVector[i]), unitNormal + normalMapVector), unitVectorToCamera));
    ...
    out_Color = (clamp(emissiveMaskColor * emissiveColor, vec4(0.0, 0.0, 0.0, 0.0), vec4(1.0, 1.0, 1.0, 1.0)) + clamp((albedoColor * (vec4(totalDiffuse + (ambient), 1.0)) + (vec4(totalSpecular, 1.0))), vec4(0.0, 0.0, 0.0, 0.0), vec4(1.0, 1.0, 1.0, 1.0)));
